...
- 上游参考项目：[qixing-jk/all-api-hub](https://github.com/qixing-jk/all-api-hub/tree/5f5b5c9db492f1a744610df1732d744ad07c22fa)，AGPL-3.0；核对 commit `5f5b5c9`（2026-07-23，3.53.0），提供多站点类型、能力化适配器、自动签到、公告聚合、余额/用量与导入导出等完整浏览器扩展能力。APIHub 只提取公开备份格式与字段语义，独立实现导入流程，没有复制其源码。
...
外部仓库信息最近访问日期为 2026-07-23；新增来源使用固定 commit 链接，避免后续默认分支变化导致证据漂移。
...
| All API Hub 备份 | 版本/账号数组兼容、类型映射、dry-run 和事务导入 | 为现有站点管理提供可审查、可回滚的迁移入口；不接管 Cookie 或浏览器状态 |
