...
func (_q *AnnouncementQuery) QuerySite() *SiteQuery {
...
func (_q *AnnouncementQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *AnnouncementQuery) FirstIDX(ctx context.Context) string {
...
func (_q *AnnouncementQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *AnnouncementQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *AnnouncementQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *AnnouncementQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *AnnouncementQuery) WithSite(opts ...func(*SiteQuery)) *AnnouncementQuery {
	query := (&SiteClient{config: _q.config}).Query()
...
//		GroupBy(announcement.FieldSiteID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Announcement.Query().
//		Select(announcement.FieldSiteID).
//		Scan(ctx, &v)
...
func (_q *AnnouncementQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Announcement, error) {
	...
	if query := _q.withSite; query != nil {
		if err := _q.loadSite(ctx, query, nodes, nil,
			func(n *Announcement, e *Site) { n.Edges.Site = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AnnouncementQuery) loadSite(ctx context.Context, query *SiteQuery, nodes []*Announcement, init func(*Announcement), assign func(*Announcement, *Site)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Announcement)
	for i := range nodes {
		fk := nodes[i].SiteID
...
func (_q *AnnouncementQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(announcement.Table, announcement.Columns, sqlgraph.NewFieldSpec(announcement.FieldID, field.TypeString))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSite != nil {
			_spec.Node.AddColumnOnce(announcement.FieldSiteID)
		}
	}
